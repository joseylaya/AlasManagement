...
class ResumeSupportAiAction
{
    public function execute(SupportConversation $conversation, User $admin): SupportConversation
    {
        $result = DB::transaction(function () use ($conversation, $admin) {
            ...
            $locked->update(['mode' => SupportConversationMode::AI_ACTIVE, 'status' => SupportConversationStatus::OPEN, 'assigned_admin_id' => null, 'ai_resumed_at' => now()]);
            SupportEvent::create(['conversation_id' => $locked->id, 'event_type' => 'AI_RESUMED', 'actor_type' => 'ADMIN', 'actor_id' => $admin->id]);
            return $locked->fresh();
        });
