...
class AiResponseService
{
    ...
    public function respondTo(SupportMessage $trigger): void
    {
        ...
        try {
            ...
            $history = $conversation->messages()->where('id', '!=', $trigger->id)->latest()->limit($settings->max_recent_messages)->get()->reverse()->map(fn ($message) => ['role' => in_array($message->sender_type, ['AI', 'ADMIN']) ? 'assistant' : 'user', 'content' => $message->content])->values()->all();
